...
关键规则：
- 原始输入必须放在 fenced code block 里，不要裸贴。否则用户输入里的 `#`、`##`、图片路径、列表或代码会被 Markdown 渲染成标题/列表。
- 优化后的完整提示词正文也必须放在 fenced code block 里，不要裸贴。否则正文里的标题、粗体或列表会影响中间输出版式。
- fenced code block 的语言可用 `text` 或 `markdown`；如果内容里包含三个反引号，请使用更长的反引号围栏。
...
**【格式说明】每个用户请求的第一条可见回复请以优化结果开头**：
- 本轮第一条面向用户的回复，**第一行**请使用 `📝 **原始输入**：`
- 原始输入正文请放进 `text` fenced code block，不要裸贴
- 然后是 `🔄 **优化后的理解**：`
- 然后是 `✅ **优化后的完整提示词**：`
- 优化后的完整提示词正文请放进 `markdown` fenced code block，不要裸贴
- 最后才是执行任务的内容
- 只展示一次对用户输入的理解，再执行任务，这样用户可以看到提示词优化过程
- 这不是每条消息的格式；展示一次后，后续进度、工具说明、Review、Verification 和最终答复不要重复“原始输入 / 优化后的理解 / 优化后的完整提示词”三段
